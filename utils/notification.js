
const admin  = require('firebase-admin');


// Create service account and generate service.json file and then put the following from service.json file
const FIREBASE_ADMIN_PRIVATE_KEY = "-----BEGIN PRIVATE KEY-----\nMIIEvwIBADANBgkqhkiG9w0B\\n-----END PRIVATE KEY-----\n"
const FIREBASE_ADMIN_CLIENT_EMAIL = "test.iam.gserviceaccount.com"
const FIREBASE_ADMIN_PROJECT_ID = "test-services-android"

admin.initializeApp({
    credential: admin.credential.cert({
      private_key: FIREBASE_ADMIN_PRIVATE_KEY,
      client_email: FIREBASE_ADMIN_CLIENT_EMAIL,
      project_id: FIREBASE_ADMIN_PROJECT_ID,
    })
});

exports.sendNotification = async(topic,title,message,image,isSilent) => {
    const messaging = admin.messaging()

    // console.log(isSilent)
    const messaget = {
        apns: {
          payload: {
            aps: {
              "content-available": isSilent, // pass 1 for silent notification 
              alert: ""
            }
          }
        },
        data:{
                "test":"test"
            },
        notification: {
            title,
            body: message,
            image:image
        },
        topic:topic
    };
    // var payload = {
    //     notification: {
    //         title,
    //         body: message,
    //         image:image
    //     },
    //     topic:"All"
    //     };

    messaging.send(payload)
    .then((result) => {
        console.log(result)
    })
}



